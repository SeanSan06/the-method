...
class ScoringConfig:
    ...
    def validate(self) -> None:
        ...
        if any(w < 0 or w > 1 for w in [self.tfidf_weight, self.skills_weight, self.experience_weight]):
...
class ResumeAnalyzer:
    ...
    def analyze(self, data: Dict[str, Any]) -> AnalysisResult:
        ...
        logger.info("Extracted %d resume skills and %d job skills", 
        ...
        # Score components
        tfidf_score = self._calculate_tfidf_score(resume, job_desc, resume_skills, job_skills)
        ...
        logger.info("Analysis complete: score=%.1f, confidence=%s, recommendation=%s",
    ...
    def _validate_input(self, data: Dict[str, Any]) -> None:
        ...
        if not isinstance(job_desc, str):
            raise ValidationError(f"Job description must be string, got {type(job_desc).__name__}")
    ...
    def _calculate_tfidf_score(self, resume: Dict[str, Any], job_desc: str,
                               resume_skills: Set[str], job_skills: Set[str]) -> float:
        """
        ...
        overlap_ratio = len(resume_skills & job_skills) / len(job_skills | resume_skills)
        
        # Bonus for keyword density in JD
        total_keywords = len(job_skills)
        skill_density = total_keywords / (len(job_desc.split()) + 1)  # +1 to avoid division by zero
    ...
    def _calculate_experience(self, experience: List[Dict[str, Any]]) -> int:
        ...
        for exp in experience:
            ...
            end_year = current_year if 'present' in end_date.lower() else (self._extract_year(end_date) or current_year)
...
def analyze_resume(data: Dict[str, Any], config: Optional[ScoringConfig] = None) -> AnalysisResult:
    """
